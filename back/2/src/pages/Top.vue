          ...
          <li
            ...
            <router-link
              ...
              <img
                ...
                alt="Avatar">
              <span>{{ user.data.profile.name }}</span>
              <svg class="icon">
                <use xlink:href="#star"/>
              </svg>
...
export default {
  ...
  mounted: function () {
    ...
    ref.once('value', (snap) => {
      list.sort((a, b) => {
        let aMax = Math.max(...a.data[this.$store.state.settings.lang].history)
        let bMax = Math.max(...b.data[this.$store.state.settings.lang].history)
        return aMax - bMax
      })
