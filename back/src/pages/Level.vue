...
export default {
  ...
  data () {
    return {
      ...
      words: this.$store.state.wordsRu,
      treasure: [],
      solutions: [],
      answersCount: 0,
      strike: 0,
  ...
  computed: {
    user () {
      return this.$store.state.users.find(x => x.id === 1)
    },
    taskLevel () {
    ...
    answersAll () {
      ...
      let answersAll = {}
      for (let index = 2; index <= maxAnswerLength; index++) {
        let answers = Task.getAnswersAll(this.words[index], this.task)
  ...
  beforeCreate: function () {
    // let users = Func.getData('/data/users.json', 'json')
    // users.then(result => {
    //   let user = result[0].find(x => x.id === 1)
    //   this.user = user
    //   let levels = result[1]
    //   this.config = result[2]
    //   let words = result[3]
    //   let taskLevel = user.taskLevel
    //   let wordLength = levels[taskLevel].wordLength
    //   let sublevels = levels[taskLevel].sublevels[user.taskSublevel]
    //   let maxAnswerLength = Object.keys(sublevels)[Object.keys(sublevels).length - 1]
    //   this.task = 'право'
    //   this.task = Task.getTask(words[wordLength], taskLevel)
    //   this.descriptions[this.task] = words[wordLength][this.task].description
    //   for (let index = 2; index <= maxAnswerLength; index++) {
    //     let answers = Task.getAnswersAll(words[index], this.task)
    //     this.answersAll[index] = answers
    //     answers.forEach(answer => {
    //       this.descriptions[answer] = words[index][answer].description
    //     })
    //   }
    //   let answers = Task.getAnswers(this.answersAll, sublevels)
