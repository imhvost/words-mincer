...
new Vue({
  ...
  watch: {
    onLine (v) {
      if (v && this.$store.state.isOfline) {
        this.$store.commit('SET_IS_OFFLINE', false)
  ...
  mounted: function () {
    ...
    if (config && words && levels) {
      if (this.onLine) {
        ...
        Func.getData('/data/config.json', 'json').then(result => {
          // console.log(wordsVersion)
          if (wordsVersion !== result.wordsVersion[this.lang]) {
            this.$store.commit('SET_CONFIG', result)
            Func.getData(wordsFile, 'json').then(result => {
