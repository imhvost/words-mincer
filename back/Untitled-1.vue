...
export default {
  ...
  computed: {
    ...
    answersAll () {
      ...
      // if (this.$store.state.answersAll) {
      //   let answers = {}
      //   Object.keys(cashAnswers).forEach(el => {
      //     let decodeAnswers = cashAnswers[el].map(e => decodeURIComponent(atob(e)))
      //     answers[el] = decodeAnswers
      //   })
      //   console.log(answers)
      //   return answers
      // }
  ...
  methods: {
    getNextLevel () {
      ...
      myWorker.onmessage = (e) => {
        ...
        this.$store.commit('NEXT_LEVEL', {task: e.data.task.enc})
        this.$store.commit('NEXT_LEVEL', {answersAll: e.data.answersAll.enc})
      }
    },
    openFirstLetters () {
    ...
    addCoins (coins) {
      ...
      if (coins > 0) {
        this.panelInfo.coins.text = '+' + coins
    ...
    goToNextLevel (type) {
      ...
      setTimeout(() => {
        if (this.$store.state.nextLevel.task) {
          this.$store.commit('SET_TASK', this.$store.state.nextLevel.task)
        } else {
        ...
        if (this.$store.state.nextLevel.answersAll) {
          this.$store.commit('SET_ANSWERS_ALL', this.$store.state.nextLevel.answersAll)
        } else {
