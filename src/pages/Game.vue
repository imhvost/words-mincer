      ...
      <div class="container">
        <div
          ...
          <div
            ...
            <ul
              :data-letters="answer.answer.length"
              class="answer-inputs">
              <li
      ...
      <div class="container">
        <div class="panel-body">
          ...
          <template v-if="!isBonus">
            <button
              type="button"
              class="panel-btn panel-btn-helper panel-btn-helper-open-first-letters"
            ...
            <button
              type="button"
              class="panel-btn panel-btn-helper panel-btn-helper-open-last-letters"
            ...
            <button
              type="button"
              class="panel-btn panel-btn-helper panel-btn-helper-bomb"
    ...
    <div
      ...
      <div class="level-done-body">
        <div class="container">
          ...
          <div
            v-if="treasure.length"
            class="level-done-state">
            <div class="level-done-state-info">
              <span>{{ text.doneState.treasure[lang] }}</span>
              <button
                :class="levelDoneTabs.treasure ? 'active': ''"
                @click="levelDoneTabs.treasure = !levelDoneTabs.treasure">{{ treasure.length }}</button>
            </div>
            <div
              v-if="levelDoneTabs.treasure"
              class="rest-tab">
              <ul
                :class="treasure.length > 1 ? 'level-done-answers-columns' : ''"
                class="level-done-answers">
                <li
                  v-for="(word, index) in treasure"
                  :key="index">{{ word }}</li>
              </ul>
            </div>
          </div>
          <div
            ...
            class="level-done-state">
            <div class="level-done-state-info">
              <span>{{ text.doneState.rest[lang] }}</span>
              <button
                :class="levelDoneTabs.rest ? 'active': ''"
                @click="levelDoneTabs.rest = !levelDoneTabs.rest">{{ getRestCount() }}</button>
            </div>
            <div
              v-if="levelDoneTabs.rest"
              class="rest-tab">
              <ul
                :class="getRestCount() > 1 ? 'level-done-answers-columns' : ''"
                class="level-done-answers">
                <li
                  v-for="(word, index) in getRest()"
                  :key="index">{{ word }}</li>
              </ul>
            </div>
          </div>
          <ul class="level-done-list">
            <li v-if="user.id === 'guest'">
              <button
                type="button"
                class="level-back-btn"
                aria-label="Back to Sign In"
                @click="$root.playAudio('click'); $router.push('home');">
                <svg class="icon">
                  <use xlink:href="#back"/>
                </svg>
                <span>{{ text.backToSignIn[lang] }}</span>
              </button>
            </li>
            <li v-else>
              <button
                type="button"
                class="level-done-btn level-done-btn-save"
            ...
            <li>
              <button
                type="button"
                class="level-done-btn level-done-btn-next"
...
export default {
  ...
  computed: {
    ...mapState(['game', 'config', 'words', 'levels', 'settings', 'awardsList']),
    lang () {
    ...
    userData () {
      if (this.$store.state.userData === null) {
        let userData = {}
        userData[this.user.id] = this.userConfig
        this.$store.commit('SET_USER_DATA', userData)
        return userData[this.user.id][this.lang]
      }
      return this.$store.state.userData[this.user.id][this.lang]
    },
    awards () {
    ...
    task () {
      ...
      if (this.$store.state.task) {
        ...
        return decodeURIComponent(atob(task))
      }
    ...
    minAnswerLength () {
      let minAnswerLength
      Object.keys(this.sublevels).forEach(el => {
        if (minAnswerLength) return
        if (this.sublevels[el] !== 0) minAnswerLength = +el
      })
      return minAnswerLength
    },
    maxAnswerLength () {
      return +Object.keys(this.sublevels)[Object.keys(this.sublevels).length - 1]
    },
    answersAll () {
      ...
      let answersAll = {}
      for (let index = this.minAnswerLength; index <= this.maxAnswerLength; index++) {
  ...
  methods: {
    ...
    openNothing (helper) {
      ...
      // console.log(helper)
      this.changePanelInfo(helper, this.text.panelInfo.openNothing[this.lang], 'minus')
    ...
    addToTreasure (word) {
      if (this.treasure.indexOf(word) === -1) {
        ...
        this.changePanelInfo('treasure', this.text.panelInfo.treasure[this.lang], 'plus')
        ...
        this.changeTaskListClass('treasure')
      }
    },
    addStrike () {
    ...
    resetStrike (type) {
      if (type === 'done') {
        this.changePanelInfo('strike', this.text.panelInfo.strike[0][this.lang], 'plus')
        ...
        if (this.userData.strike) {
          this.changePanelInfo('strike', this.text.panelInfo.strike[1][this.lang], 'minus')
        }
    ...
    addCoins (coins) {
      ...
      if (!this.panelInfo.coins.count) this.panelInfo.coins.count = 0
      this.panelInfo.coins.count += coins
      // console.log(coins, this.panelInfo.coins.count)
      if (coins > 0) {
        this.changePanelInfo('coins', '+' + this.panelInfo.coins.count, 'plus')
        ...
        this.changePanelInfo('coins', '-' + this.panelInfo.coins.count, 'minus')
      }
    ...
    updateCounterdown (stop) {
      ...
      if (stop) {
        ...
        this.changePanelInfo('time', this.text.panelInfo.time[0][this.lang], 'plus')
        this.clearPanelInfo('time')
        setTimeout(() => {
          this.changePanelInfo('coins', '+' + this.counterdown.coins, 'plus')
        }, 400)
      ...
      this.counterdown.timer = setInterval(() => {
        ...
        if (this.counterdown.time === 0) {
          ...
          this.changePanelInfo('time', this.text.panelInfo.time[1][this.lang], 'minus')
          this.clearPanelInfo('time')
          clearInterval(this.counterdown.timer)
          return
        }
    ...
    wrong (word, mistake) {
      ...
      this.changeTaskListClass('wrong')
      this.changePanelInfo('mistakes', this.text.panelInfo.mistakes[this.lang], 'minus')
    ...
    correct (answer) {
      ...
      this.changeTaskListClass('correct')
    ...
    clearPanelInfo (panel, reset) {
      ...
      if (this.panelInfo[panel].clear) return
      this.panelInfo[panel].clear = true
      setTimeout(() => {
        this.panelInfo[panel] = {}
      }, 1000)
    },
    changePanelInfo (panel, text, className, count) {
      const panelObject = this.panelInfo[panel]
      if (text) panelObject.text = text
      if (className) panelObject.className = className
      if (count) panelObject.count = count
    },
    getAnswersAllCount () {
      let count = 0
      Object.keys(this.answersAll).forEach(el => {
        count += Object.keys(this.answersAll[el]).length
      })
      return count
    },
    getRest () {
      let rest = []
      Object.keys(this.answersAll).forEach(key => {
        const answers = this.answersAll[key]
        answers.forEach(answer => {
          if (this.solutions.includes(answer) || this.treasure.includes(answer)) return
          rest.push(answer)
        })
      })
      return rest
    ...
    getRestCount () {
      return this.getAnswersAllCount() - this.solutions.length - this.treasure.length
    },
    changeTaskListClass (className) {
      this.taskListClass = className
      setTimeout(() => {
        this.taskListClass = ''
      }, 400)
    }
...
.panel-btn-helper-open-first-letters{
  color:@green;
}
.panel-btn-helper-open-last-letters{
  color:@title;
}
.panel-btn-helper-bomb{
  color:@link;
}
.panel-btn-mistakes{
  margin-right:auto;
...
.level-done-answers{
  ...
  background:linear-gradient(to top,#f1f1f1 1px,#f9f9f9 1px);
...
.level-done-btn-next{
  ...
  color:#fff;
  font-size:1.25rem;
  box-shadow:0 .5rem 1rem fade(@green,40%);
  use{
    fill:#fff;
  }
}
...
@keyframes panel-info{
  ...
  0%{
    transform:scale(1) translate3d(0,1.4286rem,0);
  }
  100%{
